...
import { LOGIN, SET_LOADER, USER_DATA } from '../../types/auth/index';
import {
    Alert,
} from 'react-native';
            ...
            .then((querySnapshot) => {
                if (querySnapshot.size < 1) {
                    ...
                    querySnapshot.forEach(async (doc) => {
                        const isValid = await BcryptReactNative.compareSync(Password, doc._data.password);
                        if (isValid) {
                            ...
                            dispatch({ type: SET_LOADER, value: false });
                            Alert.alert('Invalid User!', 'wrong passwordl', [
                                { text: 'Okay' }
                            ]);
                        }
                    });
